...
The overall model can be written as $Total_-time = \alpha + \beta_{format} + \gamma_{music} + \beta\gamma + \zeta + \eta + \epsilon$ where $\epsilon$ is random error and $\zeta$ and $\eta$ are random effects representing the text and the participant music interaction.
...
This model is $Comprehension = \alpha + \beta_{format} + \gamma_{music} + \beta\gamma + \zeta + \eta + \epsilon$ where $\epsilon$ is random error and $\zeta$ and $\eta$ are random effects for text and the music participant interaction respectively.
...
```{r, echo=FALSE, warning= FALSE, message= FALSE, fig.width=7, fig.height=2.7}
par(mfrow = c(1,2))

plot(model_time, which = 1)  
plot(model_time, which = 2)
...
plot(model_comp, which = 1) 
plot(model_comp, which = 2)
```

Additional diagnostics were performed for the mixed-effects models. Both models produced singular-fit warnings, indicating that at least one random-effect variance component was estimated near zero. In particular, the participant-specific random slope for music contributed little additional variability beyond the random intercept structure. Despite this, the mixed-effects structure was retained because it appropriately reflected the 
repeated-measures experimental design and accounted for participant-level grouping effects.

```{r, echo=FALSE, warning= FALSE, message= FALSE, fig.width=3.3, fig.height=2.5}
sing1 <- isSingular(model_time_lmer)
sing2 <- isSingular(model_comp_lmer)
